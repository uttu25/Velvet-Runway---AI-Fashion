  ...
  const handleInteract = useCallback(async () => {
    ...
    try {
      ...
      if (error.message === "QUOTA_EXHAUSTED") {
        alert("The Agency is currently busy with other VIP clients. Please wait a moment before requesting a new look.");
      } else if (error.message === "SAFETY_FILTER") {
        alert("This look is too exclusive even for us! Let's try another style.");
      } else {
        alert("Our stylists hit a roadblock. Please try again soon.");
      }
    }
  }, [model, onUpdate]);
  ...
  const handleCustomEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message === "QUOTA_EXHAUSTED") {
        ...
        alert("Our stylists couldn't process that specific request.");
      }
    }
  };
        ...
        {!showCustomInput ? (
          ...
          <form onSubmit={handleCustomEdit} className="flex flex-col gap-2">
            <input 
              ...
              placeholder="e.g., silk robe, sunglasses..."
